...
!apt-get update ###imp to install chromium driver and browser
...
chrome_binary_path = "chromedriver" # for colab use chrome driver
...
def thread_call(list):
	...
	#d = w.Chrome(executable_path=chrome_binary_path,options=options) #for internal
	d = webdriver.Chrome(executable_path=chrome_binary_path, options=options)  #for colab
	print(list)
	first = 1
	for i in list:
		res = r.get(url+str(i))
